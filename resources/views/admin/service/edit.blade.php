...
<div class="col-12">
    ...
    <form class="col-12 mx-auto" action="{{route('service.update' ,['service'=>$service])}}" method="post" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="form-row">
            ...
            <div class="form-group col-12 col-md-6 col-lg-4">
                ...
                <input name="price" class="form-control" type="number" value="{{ $service->price }}">
                        ...
                        <input type="checkbox" name="specs[]" value="{{ $spec->id }}" 
                            @foreach ($service->specs as $data)
                                    @if ($data->id == $spec->id)
                                        checked
                                    @endif
                            @endforeach> {{ucfirst($spec->description)}}
                        </div>
                    @endforeach
                </div>
            </div>
